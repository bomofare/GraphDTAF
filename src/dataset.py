...
class MyDataset(Dataset):
    def __init__(self, data_path, phase, pdbdata, max_seq_len, max_pkt_len, max_smi_len, pkt_window=None):
        ...
        self.seq_path = []
        for deal_pdb in pdbdata[phase]:
            deal_pdb = deal_pdb + '.csv'
            seq_paths = data_path / phase / 'global' / deal_pdb
            self.seq_path.append(seq_paths)
            # seq_path = data_path / phase / 'global'
        # self.seq_path = sorted(list(seq_path.glob('*')))
        # print('pkt_path==',self.seq_path)
        self.max_seq_len = max_seq_len

        self.pkt_path = []
        for deal_pdb in pdbdata[phase]:
            deal_pdb = deal_pdb + '.csv'
            seq_paths = data_path / phase / 'pocket' / deal_pdb
            self.pkt_path.append(seq_paths)

        # pkt_path = data_path / phase / 'pocket'
        # self.pkt_path = sorted(list(pkt_path.glob('*')))
        # print('pkt_path==', list(pkt_path.glob('*'))[0])
        # print('pkt_path==', len(self.pkt_path))
        self.max_pkt_len = max_pkt_len
        self.pkt_window = pkt_window
        # self.pkt_stride = pkt_stride
        # print('pkt_window', self.pkt_window)
        # print('pkt_stride', self.pkt_stride)
        # if self.pkt_window is None or self.pkt_stride is None:
        if self.pkt_window is None :
        ...
        self.length = len(self.seq_path)
        # self.length = len(self.smi)
        print('----------------------------------------')
        # # print('affinity', self.affinity['5tcy'])
        # print('smi.length==', self.length)
        # print('seq==', self.seq_path)
        # print('pkt==', self.pkt_path)
    ...
    def __getitem__(self, idx):
        ...
        # print('///////////////////////////seq', seq)
        # print('pkt', seq.name.split('.')[0])
        # print('++++++++++++++++++++++++++++++++++++++++++')
        # print('self.pdbdata[self.phase',self.pdbdata[self.phase])
        # if seq.name.split('.')[0] in self.pdbdata[self.phase]:
        #     print('///////////////////////////')
        assert seq.name == pkt.name
        # a = pd.read_csv(str(seq).replace('\\','/'))
        # print('///////////////////////////a', a)
