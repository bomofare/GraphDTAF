...
if __name__ == '__main__':
    ...
    # n_epoch = 5
    interrupt = None
    save_best_epoch = 8  # when `save_best_epoch` is reached and the loss starts to decrease, save best model parameters
    # save_best_epoch = 3
    # ↑↑↑↑↑↑↑↑↑↑↑↑↑↑
    ...
    for phase in ['training', 'validation', 'test']:
        ...
        train_loader = tgDataLoader(train_data, batch_size=16, shuffle=False)
    ...
    for epoch in range(1, n_epoch + 1):
        ...
        for _p in ['training', 'validation']:
            performance = test(model, data_loaders[_p],datas[_p], loss_function, device, False)
