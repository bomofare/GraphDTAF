...
class DeepDTAF(nn.Module):
    ...
    def __init__(self):
        ...
        self.gcn1 = GCNConv(78 * 4, 78 * 4)
        self.fc_g1 = nn.Linear(128, 128)
        self.fc_g2 = torch.nn.Linear(78 * 4 * 2, 1500)
        self.fc_g3 = torch.nn.Linear(1500, 128)
    ...
    def forward(self, seq, pkt, data):
        ...
        smi_conv = self.gcn1(smi_conv, edge_index)
        smi_conv = self.relu(smi_conv)
        smi_conv = torch.cat([gmp(smi_conv, batch), gap(smi_conv, batch)], dim=1)
        smi_conv = self.relu(self.fc_g2(smi_conv))
        smi_conv = self.dropout(smi_conv)
        smi_conv = self.fc_g3(smi_conv)
        # smi_conv = self.relu(smi_conv)
        # smi_conv = gmp(smi_conv, batch)  # global max pooling
        # smi_conv = self.fc_g1(smi_conv)
        # smi_conv = self.relu(smi_conv)
