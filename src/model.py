...
class DeepDTAF(nn.Module):
    ...
    def __init__(self):
        ...
        # self.gat5 = GATConv(num_features_xd * 4, num_features_xd, heads=4,dropout=dropout)
        # self.gcn = GCNConv(num_features_xd*4, num_features_xd*4)
        # self.fc_gc1 = torch.nn.Linear(num_features_xd * 4 * 2, 1500)
        # self.fc_gc2 = torch.nn.Linear(1500, 128)
        # self.fc_g1 = nn.Linear(smi_oc, smi_oc)

        self.conv1 = GATConv(num_features_xd, num_features_xd, heads=10)
        self.conv2 = GCNConv(num_features_xd * 10, num_features_xd * 10)
        self.fc_g1 = torch.nn.Linear(num_features_xd * 10 * 2, 1500)
        self.fc_g2 = torch.nn.Linear(1500, 128)
    ...
    def forward(self, seq, pkt, data):
        ...
        # smi_conv = torch.cat([gmp(smi_conv, batch), gap(smi_conv, batch)], dim=1)
        # smi_conv = self.relu(self.fc_g1(smi_conv))
        # smi_conv = self.dropout(smi_conv)
        # smi_conv = self.fc_g2(smi_conv)
        ...
        smi_conv = self.conv1(smi_conv, edge_index)
        smi_conv = self.relu(smi_conv)
        smi_conv = self.conv2(smi_conv, edge_index)
        smi_conv = self.relu(smi_conv)
